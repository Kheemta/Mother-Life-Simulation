           ...
           var colliders = Physics.OverlapSphere(transform.position, maxExplosionRadius, applyDamageLayer);
            ...
            for (int i = 0; i < colliders.Length; ++i)
            {
                if (colliders[i] != null && colliders[i].gameObject != null && !collidersReached.Contains(colliders[i].gameObject))
                {
                    ...
                    //Emerald AI Damage v Explosive

                    // azhar add
                    // if (colliders[i].gameObject.GetComponentInParent<EmeraldAI.EmeraldAISystem>() )
                    // {

                    //     colliders[i].gameObject.GetComponentInParent<EmeraldAI.EmeraldAISystem>().Damage(damage.damageValue, EmeraldAI.EmeraldAISystem.TargetType.Player);
                    //     colliders[i].gameObject.GetComponentInParent<Animator>().enabled = false;
                    //     //StartCoroutine(MyDelay(colliders[i].gameObject.GetComponentInParent<Animator>(), 0.01f));
                    //     //colliders[i].gameObject.GetComponentInParent<Animator>().enabled = false;
