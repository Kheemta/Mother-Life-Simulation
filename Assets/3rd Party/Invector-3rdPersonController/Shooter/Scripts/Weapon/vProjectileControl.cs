        ...
        protected virtual void Update()
        {
            ...
            if (Physics.Linecast(previousPosition, transform.position + transform.forward * 0.5f, out hitInfo, hitLayer))
            {
                ...
                if (!(ignoreTags.Contains(hitInfo.collider.gameObject.tag) || (shooterTransform != null && hitInfo.collider.transform.IsChildOf(shooterTransform))))
                {
                    ...
                    //Emerald Damage ...
                    // if (hitInfo.collider.gameObject.GetComponentInParent<EmeraldAI.EmeraldAISystem>() != null)
                    // {
                    //     //Debug.LogError("Hit Done");
                      
                    //     //if(hitInfo.collider.gameObject.GetComponent<ZoombieController>())
                    //         hitInfo.collider.gameObject.GetComponentInParent<ZoombieController>().Bodyhitpoints(hitInfo);
                    // }
